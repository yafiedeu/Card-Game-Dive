...
class CardImageLoaderTest {
    ...
    fun testLoadAll() {
        ...
        CardSuit.values().forEach { suit ->
            CardValue.values().forEach { value ->
                allImages += imageLoader.frontImageFor(suit, value).image
            }
        }
        allImages += imageLoader.backImage.image
        allImages += imageLoader.blankImage.image
    ...
    fun testCardEquality() {
        val testImage = imageLoader.frontImageFor(CardSuit.HEARTS, CardValue.QUEEN).image
    ...
    fun testCardUnequality() {
        val testImage = imageLoader.frontImageFor(CardSuit.SPADES, CardValue.ACE).image
